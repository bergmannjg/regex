...
for (;;)
  ...
  for (;;)
    ...
    next if $_ =~ /^\\=(?:\s|$)/;   # Comment line
	  ...
	  printf $outfile "%s", "${jsonMarker}{\n" if $json;
	  printf $outfile "%s", "${jsonMarker}  \"matchExpected\": $match_expected,\n" if $json;
    $mypattern = encode_json $pattern;
	  printf $outfile "%s", "${jsonMarker}  \"pattern\": $mypattern,\n" if $json;
    $myhaystack = encode_json $_;
    printf $outfile "%s", "${jsonMarker}  \"haystack\": $myhaystack,\n" if $json;
    ...
    if ($@)
      ...
      {
      printf $outfile "${jsonMarker}  \"match\": [\n" if $json;
      while (scalar(@subs) != 0)
        {
        printf $outfile (" 0: %s\n", &pchars($subs[0]));
        $match = encode_json &pchars($subs[0]);
        printf $outfile "${jsonMarker}    {\"match\": $match\n" if $json;
        ...
        for ($i = 1; $i <= 16; $i++)
          ...
          if (defined $subs[$i])
            ...
            printf $outfile ("%2d: %s\n", $i, &pchars($subs[$i]));
            $match = encode_json &pchars($subs[$i]);
            printf $outfile ("${jsonMarker}    ,\"group%d\": %s\n", $i, $match) if $json;
            $last_printed = $i;
            }
          }
        splice(@subs, 0, 18);
