...
/-- State of the parser -/
protected structure Parser where
...
protected def parsePosixCharacterClass (p : String) (x : Syntax) : ParserM ClassSetItem := do
  match x with
...
protected def parseGenericCharacterType (p : String) (x : Syntax) : ParserM Ast := do
  match AstItems.valuesOfLitSyntax x with
  | some (_, (.synthetic f t), s) =>
      -- ClassUnicodeKind.Named
      let (negated, kind) ← AstItems.getPerlClass (← AstItems.firstChar s)
      pure $ Ast.ClassPerl ⟨⟨p, f, t⟩, kind, negated⟩
  | _ => Except.error s!"no GenericCharacterType values in {x}"

protected def parseUnicodeCharacterProperty (p : String) (x : Syntax) : ParserM Ast := do
  match x with
  | Syntax.node (.synthetic f t) kind arr =>
      ...
      | [.atom _ s] =>
        if s.length = 1
        then pure $ Ast.ClassUnicode ⟨⟨p, f, t⟩ , negated, ClassUnicodeKind.OneLetter (← AstItems.firstChar s)⟩
...
protected def parseBackReference (p : String) (x : Syntax) : ParserM Ast := do
  let state ← get
  match AstItems.valuesOfLitSyntax x with
...
protected def parseBackReferenceOrLiteral (p : String) (x : Syntax) : ParserM Ast := do
  let state ← get
  match x with
  | Syntax.node (.synthetic f t) `backReferenceNumberOrLiteral #[b, l] =>
    let (b, c) ←
      match b, l with
      | .atom .none b, .atom .none s => pure (b, ← AstItems.firstChar s)
...
protected def parseAssertion (p : String) (x : Syntax) : ParserM Ast := do
  let flavor ← read
...
protected def toFlags (chars : List Char) : ParserM $ Array FlagsItem := do
...
protected def parseGroupKind (p : String) (x : Syntax) : ParserM GroupKind := do
  match AstItems.valuesOfLitSyntax x with
  ...
  | some (`nonCapturingGroup, (.synthetic f t), s) =>
      pure $ GroupKind.NonCapturing ⟨⟨p, f, t⟩, ← AstItems.toFlags s.toList⟩
...
protected def toConcat (asts : Array Ast) : Ast :=
  match asts with | #[ast] => ast | _ => .Concat ⟨"", asts⟩

protected def toRepetitionKind (p l r : String) : ParserM RepetitionKind := do
...
protected def toRepetition (p : String) (f t : String.Pos.Raw) (l r m : String) (ast : Ast)
    : ParserM Ast := do
  let (greedy, possessive) :=
    ...
    | _ => (true, false)
  pure $
    Ast.Repetition $ Repetition.mk ⟨p,f, t⟩ ⟨⟨p,f, t⟩, ← AstItems.toRepetitionKind p l r⟩ greedy possessive ast

protected def toClassSetItem (ast : Ast) : ParserM ClassSetItem := do
  match ast with
...
protected def rangeToClassSetItem (p : String) (a b : Syntax) : ParserM ClassSetItem := do
  match AstItems.valuesOfLitSyntax a , AstItems.valuesOfLitSyntax b  with
...
protected def set_width (pattern : String) (g : Group) : ParserM Group := do
  match g with
...
protected def unfoldUnion (union : ClassSetUnion) : Sum ClassSetItem ClassSetBinaryOp :=
...
protected def toNode? (t : Syntax)
    : Option { x : SourceInfo × SyntaxNodeKind × Array Syntax // sizeOf x.2.2 < sizeOf t } :=
  match t with
  | .node si kind args => some ⟨(si, kind, args), by simp; omega⟩
  | _ => none

protected def parseSimpleClassSetItem (p : String) (x : Syntax) (kind : SyntaxNodeKind)
    ( arr : Array Syntax) : ParserM $ Option ClassSetItem := do
  match kind with
  | `whitespace => pure $ ClassSetItem.Empty ""
  |  `literal => pure (← AstItems.parseLiteral p x >>= AstItems.toClassSetItem)
  |  `hyphen => pure (← AstItems.parseHyphen p x >>= AstItems.toClassSetItem)
  |  `range => pure (← AstItems.rangeToClassSetItem p arr[0]! arr[1]!)
  |  `genericCharacterType => pure (← AstItems.parseGenericCharacterType p x >>= AstItems.toClassSetItem)
  |  `posixCharacterClass => AstItems.parsePosixCharacterClass p x
  |  `endQuote => throw (toError p .EndQuoteWithoutOpenQuote)
  |  `unicodeCharacterProperty => pure (← AstItems.parseUnicodeCharacterProperty p x >>= AstItems.toClassSetItem)
  |  `unicodeCharacterPropertyNegated => pure (← AstItems.parseUnicodeCharacterProperty p x >>= AstItems.toClassSetItem)
  | _ => pure none

protected def parseSimpleVal (p : String) (x : Syntax) : ParserM $ Option Ast := do
  match x.getKind with
  | `whitespace => pure Ast.Empty
  | `comment => pure Ast.Empty
  | `literal => AstItems.parseLiteral p x
  | `dot => AstItems.parseDot p x
  | `simpleAssertion => AstItems.parseAssertion p x
  | `backReferenceNumber => AstItems.parseBackReference p x
  | `backReferenceName => AstItems.parseBackReference p x
  | `backReferenceNumberOrLiteral => AstItems.parseBackReferenceOrLiteral p x
  | `genericCharacterType => AstItems.parseGenericCharacterType p x
  | `unicodeCharacterProperty => AstItems.parseUnicodeCharacterProperty p x
  | `unicodeCharacterPropertyNegated => AstItems.parseUnicodeCharacterProperty p x
  | `endQuote => throw  (toError p .EndQuoteWithoutOpenQuote)
...
protected def parseClassSetItem (p : String) (x : Syntax) : ParserM ClassSetItem := do
  match AstItems.toNode? x with
  | some x' =>
    match ← AstItems.parseSimpleClassSetItem p x x'.val.2.1 x'.val.2.2 with
    | some item => pure item
    | none =>
      match x'.val.2.1 with
      | `characterClass  => AstItems.parseCharacterClass p x'.val.2.2 >>= AstItems.toClassSetItem
      | `characterClassSetOperation => AstItems.parseCharacterClassSetOp p x'.val.2.2 >>= AstItems.toClassSetItem
      | _ => Except.error s!"unexpected class set item {x}"
  | _ =>  Except.error s!"unexpected class set item {x}"
termination_by sizeOf x

protected def parseVal (p : String) (x : Syntax) : ParserM Ast := do
  match ← AstItems.parseSimpleVal p x with
  | some x => pure x
  | none =>
    match x.getKind with
    | `group =>
        if let some ⟨((.synthetic f t), `group, arr), _⟩ := AstItems.toNode? x then AstItems.parseGroup p f t arr
        else Except.error s!"ill-formed group syntax {x}"
    | `alternatives =>
        if let some ⟨(_, `alternatives, arr), _⟩ := AstItems.toNode? x then AstItems.parseAlternatives p arr
        else Except.error s!"ill-formed alternatives syntax {x}"
    | `repetition =>
        if let some ⟨((.synthetic f t), `repetition, arr), _⟩ := AstItems.toNode? x then AstItems.parseRepetition  p f t arr
        else Except.error s!"ill-formed repetition syntax {x}"
    | `sequence =>
        if let some ⟨(_, `sequence, arr), _⟩ := AstItems.toNode? x then AstItems.parseSequence p arr
        else Except.error s!"ill-formed sequence syntax {x}"
    | `characterClass =>
        if let some ⟨(_, `characterClass, arr), _⟩ := AstItems.toNode? x then AstItems.parseCharacterClass p arr
        else Except.error s!"ill-formed sequence syntax {x}"
    | _ => Except.error s!"ill-formed val syntax {x}"
termination_by sizeOf x

protected def folder (p : String) (acc : Array Ast) (x : Syntax)
  (parse : String → Syntax → ParserM Ast) : ParserM $ Array Ast:= do
  let ast ← parse p x
  pure $ acc ++ #[ast]

protected def fold (p : String) (arr : Array Syntax) : ParserM $ Array Ast := do
  arr.attach.foldlM (fun acc (h : { x // x ∈ arr}) =>
        AstItems.folder p acc h.val (fun _ _ =>
          have : sizeOf h.val < sizeOf arr := Array.sizeOf_lt_of_mem h.property
          AstItems.parseVal p h.val)) #[]
termination_by sizeOf arr

protected def parseGroup (p : String) (f t : String.Pos.Raw) (arr : Array Syntax) : ParserM Ast := do
  match h : arr.head? with
  | some (kind, arr') =>
    have : sizeOf arr' < sizeOf arr := Array.sizeOf_head?_of_tail h
    match ← AstItems.parseGroupKind p kind, arr'.size with
    | GroupKind.NonCapturing flags, 0 => pure $ Ast.Flags ⟨"", flags⟩
    | kind , _ =>
        let group := Syntax.AstItems.Group.mk ⟨p, f, t⟩ kind (AstItems.toConcat (← AstItems.fold p arr'))
        let group ← AstItems.set_width p group
        pure $ Ast.Group group
  | _ => Except.error "group array is empty"
termination_by sizeOf arr

protected def parseAlternatives (p : String) (arr : Array Syntax) : ParserM Ast := do
...
protected def parseRepetition (p : String) (f t : String.Pos.Raw) (arr : Array Syntax) : ParserM Ast := do
  match hsize : arr.size, arr[0]?, arr[1]?, arr[2]?, hs : arr[3]? with
  | 4, some left, some right, some modifier, some s =>
    match AstItems.valueOfLitSyntax left `repetitionLeft,
      AstItems.valueOfLitSyntax right `repetitionRight,
      AstItems.valueOfLitSyntax modifier `repetitionModifier with
    | some l, some r, some m =>
      have : sizeOf s < sizeOf arr := by
        simp_all +zetaDelta
        rw [← hs]
        exact Array.sizeOf_get arr 3 (by omega)
      pure $ (← AstItems.toRepetition p f t l r m (← AstItems.parseVal p s))
    | _, _, _ => Except.error s!"ill-formed repetition parts {left} {right}"
  | _, _, _, _, _ => Except.error s!"ill-formed repetition array {f}:{t} {arr}"
termination_by sizeOf arr

protected def innerParseCharacterClassSetOp (p op : String) (left right : Array Syntax) : ParserM Ast := do
  let itemsLeft ← left.attach.mapM (fun (item : { x // x ∈ left }) =>
    have : sizeOf item.val < sizeOf left := Array.sizeOf_lt_of_mem item.property
    AstItems.parseClassSetItem p item.val)
  let itemsRight ← right.attach.mapM (fun (item : { x // x ∈ right }) =>
    ...
    AstItems.parseClassSetItem p item)
...
protected def parseCharacterClassSetOp (p : String) (arr : Array Syntax) : ParserM Ast :=
  let arr' := arr.attach
  ...
  | 3, some op, some left', some right' =>
    ...
    | some op, Syntax.node _ `first largs, Syntax.node _ `second rargs =>
      ...
      AstItems.innerParseCharacterClassSetOp p op largs rargs
    | _, _, _ => Except.error s!"ill-formed characterClassSetOp {arr}"
  | _, _, _, _ => Except.error s!"ill-formed characterClassSetOp {arr}"
termination_by sizeOf arr

protected def parseCharacterClass (p : String) (arr : Array Syntax) : ParserM Ast := do
  let (negated, arr') := AstItems.extractNegated arr
  let items ← arr'.val.attach.mapM (fun (h : { x // x ∈ arr'.val}) =>
    have : sizeOf h.val < sizeOf arr'.val := Array.sizeOf_lt_of_mem h.property
    AstItems.parseClassSetItem p h.val)
  let cls : ClassBracketed :=
    match AstItems.unfoldUnion ⟨"", items⟩ with
    |.inl item => ⟨"", negated, ClassSet.Item item⟩
    |.inr op => ⟨"", negated, ClassSet.BinaryOp op⟩
  pure $ Ast.ClassBracketed cls
termination_by sizeOf arr

protected def parseSequence (p : String) (arr : Array Syntax) : ParserM Ast := do
  let ast ← arr.attach.foldlM (fun acc (h : { x // x ∈ arr}) =>
        AstItems.folder p acc h.val (fun _ _ =>
          have : sizeOf h.val < sizeOf arr := Array.sizeOf_lt_of_mem h.property
          AstItems.parseVal p h.val)) #[]

  pure $ AstItems.toConcat ast
termination_by sizeOf arr

end

protected def parseSequence' (p : String) (x : Syntax) : ParserM Ast := do
  match x with
  | Syntax.node _ `sequence arr => AstItems.parseSequence p arr
...
def parse (p : String)  (flavor : Syntax.Flavor) (extended : Regex.Grammar.ExtendedKind := .None) : Except String Ast := do
  ...
  let (ast, parser) ← AstItems.parseSequence' p stx.raw flavor default
