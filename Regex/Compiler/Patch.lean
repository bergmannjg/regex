...
def patch («from» «to» : Unchecked.StateID) : PatchM Unit := do
  ...
  if h : «from» < states.size then
    ...
    | .Capture role _ pattern_id group_index slot isValid =>
        set (states.set «from» (Unchecked.State.Capture role «to» pattern_id group_index slot isValid))
...
theorem all_set_next_of_lt (n : Unchecked.StateID) (s : Unchecked.State)
...
theorem eat_lt («from» s : Nat) (n : Unchecked.StateID) (m : Unchecked.EatMode)
...
theorem change_frame_step_lt («from» : Nat) (f t : Unchecked.StateID)
  (states : Array Unchecked.State) (h : «from» < states.size)
...
theorem binary_union_lt («from» : Nat) (alt1 alt2 : Unchecked.StateID)
  (states : Array Unchecked.State) (h : «from» < states.size)
...
theorem union_lt («from» to : Nat) (alternates : Array Unchecked.StateID)
  (states : Array Unchecked.State) (h : «from» < states.size)
...
theorem union_reverse_lt («from» to : Nat) (alternates : Array Unchecked.StateID)
  (states : Array Unchecked.State) (h : «from» < states.size)
  (ht : to < states.size)
