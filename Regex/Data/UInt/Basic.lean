...
theorem val_ne_of_ne {c d : UInt32} (h : Not (Eq c d)) : Not (Eq c.toFin d.toFin) :=
  fun h' => absurd (UInt32.eq_of_toFin_eq h') h
...
theorem toBitVec_add_ofNat_eq {n m : Nat}
    ...
    = BitVec.ofNat 32 ((ofNat n).toFin + (ofNat m).toFin) := by
...
theorem ofNat_add_ofNat (n m : Nat) (hnm : n + m < UInt32.size)
  ...
  simp only [← UInt32.add_def', UInt32.add, toBitVec_add_ofNat_eq, UInt32.ofBitVec_ofNat]
...
theorem toNat_add_toNat (n m : UInt32) (hnm : n.toFin + m.toFin < UInt32.size)
...
theorem toNat_sub_toNat {n m : UInt32} (hmn : m.toFin ≤ n.toFin) (h2 : n.toFin < UInt32.size)
    : n.toNat - m.toNat = (n - m).toNat := by
  have h1 : n.toFin - m.toFin < UInt32.size := Nat.le_lt_sub_lt hmn h2
  have h2 : m.toFin < UInt32.size := Nat.lt_of_le_of_lt hmn h2
  rw [← UInt32.sub_def']
  unfold UInt32.sub
  unfold UInt32.toNat
  simp [Fin.sub_def]
...
theorem isValidChar_succ_lt_uintSize (u : UInt32) (h : UInt32.isValidChar u)
    : u.toFin.val + 1 < UInt32.size := by
  have hx : u.toFin.val < 0x110000 := UInt32.isValidChar_lt_0x110000 u h
  have hy : u.toFin.val + 1 < 0x110000 + 1 := Nat.add_lt_add_right hx 1
  have hz : 0x110000 + 1 < UInt32.size := by simp +arith +decide
  exact Nat.lt_trans hy hz

theorem isValidChar_pred_lt_uintSize (u : UInt32) : u.toFin - 1 < UInt32.size := by
  omega
