...
theorem ofNat_eq (n : Nat) (h : n < UInt32.size)
    : (Fin.ofNat n : Fin UInt32.size) = ⟨n, h⟩  := by
...
theorem ofNat_val_eq (n : Nat) (h : n < UInt32.size)
    : (Fin.ofNat n : Fin UInt32.size).val = n := by
  simp [Fin.ofNat_eq n h]

theorem ofNat_add (n m : Nat) (h : n + m < UInt32.size)
    : (Fin.ofNat (n + m) : Fin UInt32.size).val = n + m := by
  have hu : UInt32.size = 4294967295 + 1 := by simp_arith
  rw [hu] at h
  unfold Fin.ofNat
  simp [Fin.val_mk h]
...
theorem toNat_ofNat_eq (n : Fin UInt32.size) : (Fin.ofNat (n.val)) = n := by
  unfold Fin.ofNat
