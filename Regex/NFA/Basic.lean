...
@[grind] def nextOf (m : EatMode) : Nat :=
  match m with
  |.Until next => next
  |.ToLast next => next
...
@[grind] def nextOf (s : State) : StateID :=
  match s with
  | .Empty next => next
...
def toString (nfa : NFA) : String :=
  ...
  let states := String.join (states.toList |> List.map (fun (i, s) =>
      let iv := String.ofList (Nat.toDigits 10 i)
      "\n  " ++ iv ++ ": " ++ s.toString))
  s!"states {states}"
...
def toCkeckedState (s : Unchecked.State) (n : Nat) (h : Unchecked.State.nextOf s < n)
    : Checked.State n :=
  match s with
...
def toFun (states : Array Unchecked.State) (h : ∀ (i : Nat) _, states[i].nextOf < states.size)
    : { x // x ∈ states } → Checked.State states.size :=
  have nextOf_lt_of_mem (s : { x // x ∈ states }) : s.val.nextOf < states.size := by
    have ⟨i, ⟨hlt, _⟩⟩ := Array.getElem_of_mem s.property
    have := h i hlt
    simp_all
  fun s => toCkeckedState s states.size (nextOf_lt_of_mem s)
